...
echo "Cliente: " . $_GET['cl'] . "<br>"; // Cambiamos REQUEST por GET
echo "Factura: " . $_GET['f'] . "<br>"; // Cambiamos REQUEST por GET

if (empty($_GET['cl']) || empty($_GET['f'])) { // Cambiamos REQUEST por GET
	echo "No es posible generar la factura.";
} else {
	$codCliente = $_GET['cl']; // Cambiamos REQUEST por GET
	$noFactura = $_GET['f']; // Cambiamos REQUEST por GET
	$anulada = '';
	...
	if ($result > 0) {
		...
		ob_start();
		include(dirname(__FILE__) . '/factura.php'); // Ruta actual para el archivo factura.php
		$html = ob_get_clean();
