...
if ($update_id) {
    $stmt = mysqli_prepare($conection, "SELECT * FROM recetas WHERE id = ?");
    mysqli_stmt_bind_param($stmt, "i", $update_id);
    mysqli_stmt_execute($stmt);
    $query = mysqli_stmt_get_result($stmt);
    $update_recipe = mysqli_fetch_assoc($query);
    ...
    <script>
        ...
        function updateIngredientsList() {
            ...
            ingredients.forEach(ingredient => {
                ...
                row.innerHTML = `
                <div class="column quantity">${ingredient.quantity}($${(ingredient.quantity * ingredient.price).toFixed(2)})</div>
